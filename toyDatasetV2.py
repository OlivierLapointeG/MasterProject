...
for seed in seeds:
    ...
    if model_type == "SRDF":
        ...
        model = 'SRDF'
    if model_type == "TOGNN":
        modell = TOGNNdev.GNN(dim_in, width, layer, layersType=model, task='regression', 
                        num_classes=num_labels, weighted=weighted, residual=residual, 
                        self_loop=self_loop, double_mlp=double_mlp, mlp_depth=mlp_depth)
        model = 'TOGNN'
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        for data in train_loader:
            ...
            out_smooth, out_squash, diric = wModel(data.x.to(torch.float32), data.edge_index, batch=data.batch,
                                                     ...
                                                     edge_weights=data.weightedEdges.to(torch.float32),
        ...
        for data in test_loader:
            data = data.to(device)
            out_smooth, out_squash, diric = wModel(data.x.to(torch.float32), data.edge_index, batch= data.batch, 
                                            dirichlet=track_dirichlet, ctrl=data.ctrl, dropout=False, edge_weights=data.weightedEdges.to(torch.float32),
                                             edge_attr=data.edge_attr.to(torch.float32))
            out_smooth, out_squash = out_smooth.view(-1), out_squash.view(-1)
        ...
        for data, data2 in zip(test_loader, epsilon_loader):
            ...
            out_smooth, out_squash, _ = wModel(data.x.to(torch.float32), data.edge_index, batch= data.batch, dirichlet=False, ctrl=data.ctrl,
                                        edge_weights=data.weightedEdges.to(torch.float32), edge_attr=data.edge_attr.to(torch.float32))
            out_smooth, out_squash = out_smooth.view(-1), out_squash.view(-1)
            out_smooth2, out_squash2, _ = wModel(data2.x.to(torch.float32), data2.edge_index, batch= data2.batch, dirichlet=False,
                                                 ctrl=data2.ctrl,edge_weights=data.weightedEdges.to(torch.float32), edge_attr=data2.edge_attr.to(torch.float32))
    ...
    #take a random graph from the test set
    # data_ = random.choice(train_dataset)
